...
# hxq added
def boostgem(x, p=3, eps=1e-6):
    return (x.clamp(min=eps).pow(p).sum(-1).sum(-1) / (x > 0).sum(-1).sum(-1).type_as(x).clamp(1)).pow(1./p)


def learnpool(x, p=3, eps=1e-6):
    p_all = []
    p_all.append(l2n(mac(x)).permute(0, 2, 1, 3))
    p_all.append(l2n(spoc(x)).permute(0, 2, 1, 3))
    p_all.append(l2n(gem(x, p=3, eps=1e-6)).permute(0, 2, 1, 3))
    # p_all.append(l2n(gem(x, p=2, eps=1e-6)).permute(0, 2, 1, 3))
    # p_all.append(l2n(gem(x, p=4, eps=1e-6)).permute(0, 2, 1, 3))
    # p_all.append(l2n((x > 0).sum(-1).sum(-1).unsqueeze(-1).unsqueeze(-1).type_as(x)).permute(0, 2, 1, 3))
    p = torch.cat(tuple(p_all), 1)
    learn = torch.nn.Conv2d(len(p_all), 1, kernel_size=1).cuda()
    learn.weight.data = torch.ones(1, len(p_all), 1, 1).cuda()
    return learn(p).permute(0, 2, 1, 3)


def multipool(x, p=3, eps=1e-6):
    p_all = []
    p_all.append(l2n(mac(x)))
    p_all.append(l2n(spoc(x)))
    p_all.append(l2n(gem(x, p=3, eps=1e-6)))
    # p_all.append(l2n(gem(x, p=2, eps=1e-6)).permute(0, 2, 1, 3))
    # p_all.append(l2n(gem(x, p=4, eps=1e-6)).permute(0, 2, 1, 3))
    # p_all.append(l2n((x > 0).sum(-1).sum(-1).unsqueeze(-1).unsqueeze(-1).type_as(x)).permute(0, 2, 1, 3))
    return torch.cat(tuple(p_all), 1)
