...
#
#     def __init__(self, p=3, eps=1e-6):
#         super(MultiGeM, self).__init__()
#         self.p = Parameter(torch.ones(1) * p)
#         self.eps = eps
#
#     def forward(self, x, x2):
#         p1 = LF.gem(x, p=self.p, eps=self.eps)
#         p2 = LF.gem(x2, p=self.p, eps=self.eps)
#         # p3 = LF.gem(x3, p=self.p, eps=self.eps)
...
class BoostGeM(nn.Module):

    def __init__(self, p=3, eps=1e-6):
        super(BoostGeM, self).__init__()
        self.p = Parameter(torch.ones(1) * p)
        self.eps = eps

    def forward(self, x):
        return LF.boostgem(x, p=self.p, eps=self.eps)
...
class LearnPool(nn.Module):
    ...
    def forward(self, x):
        return LF.learnpool(x, p=self.p, eps=self.eps)
...
class MultiPool(nn.Module):
    ...
    def forward(self, x):
        return LF.multipool(x, p=self.p, eps=self.eps)
