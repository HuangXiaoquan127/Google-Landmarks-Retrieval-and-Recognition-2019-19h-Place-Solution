...
def _ReadImageList(list_path):
  ...
  """
  df_train = pd.read_csv(list_path)
  train_id = np.array(df_train['id'])
  train_id_list = train_id.tolist()
  image_paths = [os.path.join('/media/iap205/Data4T/Datasets/google-landmarks-dataset-v2/train',
                              '/'.join(list(train_id_list[i])[:3]), train_id_list[i] + '.jpg')
                 for i in range(len(train_id_list))]

  # image_paths = image_paths[3132914:]
  # image_paths = image_paths[2132914:3132914]
  # image_paths = image_paths[1132914:2132914]
  image_paths = image_paths[:1132914]
...
def main(unused_argv):
  ...
  with tf.Graph().as_default():
    ...
    # TODO: limit GPU memory use to add more task working in parallel
