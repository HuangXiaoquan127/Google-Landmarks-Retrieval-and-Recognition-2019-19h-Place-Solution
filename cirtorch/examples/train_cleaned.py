...
parameters:
train_cleaned:
/media/iap205/Data/Export/cnnimageretrieval-google_landmark_retrieval/trained_network/train_cleaned_test/R101_SPoC_FC512_GLD2_cleaned_362
--gpu-id 0 --training-dataset GLD-v2-cleaned --test-datasets google-landmarks-dataset-v2-test,roxford5k,rparis6k
--arch resnet101 --pool spoc --loss contrastive --loss-margin 0.85 --optimizer adam --lr 5e-7 --weight-decay 0 --neg-num 5
--query-size=2000 --pool-size=22000 --batch-size 5 --epochs 200 --image-size 362 --no-val
--multi-layer-cat 1 --whitening --resume latest.pth.tar

train_cleaned_method2:
/media/iap205/Data/Export/cnnimageretrieval-google_landmark_retrieval/trained_network/train_cleaned_test/R101_SPoC_FC512_GLD2_cleaned_m2_362
--gpu-id 1 --training-dataset GLD-v2-cleaned-m2 --test-datasets google-landmarks-dataset-v2-test,roxford5k,rparis6k
--arch resnet101 --pool spoc --loss contrastive --loss-margin 0.85 --optimizer adam --lr 5e-7 --weight-decay 0 --neg-num 5
--query-size=2000 --pool-size=22000 --batch-size 5 --epochs 200 --image-size 362 --no-val
--multi-layer-cat 1 --whitening --resume latest.pth.tar

train_no_cleaned:
/media/iap205/Data/Export/cnnimageretrieval-google_landmark_retrieval/trained_network/train_cleaned_test/R101_SPoC_FC512_GLD2_362
--gpu-id 2 --training-dataset google-landmarks-dataset-v2 --test-datasets google-landmarks-dataset-v2-test,roxford5k,rparis6k
--arch resnet101 --pool spoc --loss contrastive --loss-margin 0.85 --optimizer adam --lr 5e-7 --weight-decay 0 --neg-num 5
--query-size=2000 --pool-size=22000 --batch-size 5 --epochs 200 --image-size 362 --no-val
--multi-layer-cat 1 --whitening --resume latest.pth.tar

train_120k:
/media/iap205/Data/Export/cnnimageretrieval-google_landmark_retrieval/trained_network/train_cleaned_test/R101_SPoC_FC512_120k_362
--gpu-id 3 --training-dataset retrieval-SfM-120k --test-datasets google-landmarks-dataset-v2-test,roxford5k,rparis6k
--arch resnet101 --pool spoc --loss contrastive --loss-margin 0.85 --optimizer adam --lr 5e-7 --weight-decay 0 --neg-num 5
--query-size=2000 --pool-size=22000 --batch-size 5 --epochs 200 --image-size 362 --no-val
--multi-layer-cat 1 --whitening --resume latest.pth.tar

train_GLD1_resize:
/media/iap205/Data/Export/cnnimageretrieval-google_landmark_retrieval/trained_network/train_cleaned_test/R101_SPoC_FC512_GLD1_resize_362
--gpu-id 2 --training-dataset google-landmarks-dataset-resize --test-datasets google-landmarks-dataset-v2-test,roxford5k,rparis6k
--arch resnet101 --pool spoc --loss contrastive --loss-margin 0.85 --optimizer adam --lr 5e-7 --weight-decay 0 --neg-num 5
--query-size=2000 --pool-size=22000 --batch-size 5 --epochs 200 --image-size 362 --no-val
--multi-layer-cat 1 --whitening --resume latest.pth.tar
...
training_dataset_names = ['retrieval-SfM-120k', 'google-landmarks-dataset-resize', 'google-landmarks-dataset-v2',
                          'GLD-v2-cleaned', 'GLD-v2-cleaned-m2']
test_datasets_names = ['oxford5k', 'paris6k', 'roxford5k', 'rparis6k', 'google-landmarks-dataset-resize-test',
...
# hxq modified
pool_names = ['mac', 'spoc', 'gem', 'boostgem', 'learnpool', 'multipool']
...
parser.add_argument('--arch', '-a', metavar='ARCH', default='resnet101', choices=model_names.append('resnext101_32x8d'),
...
# hxq added
parser.add_argument('--multi-layer-cat', '-m', default=1, type=int, metavar='N',
                    help='number of layer to pooling and concatenate (default: 1)')
...
def main():
    ...
    # TODO: maybe pass as argument in future implementation?
    # In the research phase, do not use learning rate decay
    ...
    log_path = os.path.join(args.directory, 'results_log.csv')
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.test_freq == 0:
            with torch.no_grad():
                map_record_all = test(args.test_datasets, model)
                log.update(map_record_all)

        log = pd.DataFrame(log, index=[0])

        if os.path.isfile(log_path):
            log.to_csv(log_path, header=False, index=False, mode='a')
        else:
            log.to_csv(log_path, header=True, index=False)
        ...
        with torch.no_grad():
            ...
            torch.save({
                ...
                'min_loss': min_loss,
                'optimizer': optimizer.state_dict(),
            }, os.path.join(args.directory, 'latest.pth.tar'))
            ...
            if is_best:
                torch.save({
                    ...
                    'min_loss': min_loss,
                    'optimizer': optimizer.state_dict(),
                }, os.path.join(args.directory, 'model_best.pth.tar'))

            log_history = pd.read_csv(log_path, float_precision='round_trip')
            if 'GLD2_mAP' in log_history.columns.tolist() and \
                    log_history['GLD2_mAP'].values[-1] == log_history['GLD2_mAP'].max():
                torch.save({
                    'epoch': epoch + 1,
                    'meta': model.meta,
                    'state_dict': model.state_dict(),
                    'min_loss': min_loss,
                    'optimizer': optimizer.state_dict(),
                }, os.path.join(args.directory, 'GLD2_mAP_best.pth.tar'))
            if 'RO_M_mAP' in log_history.columns.tolist() and \
                    log_history['RO_M_mAP'].values[-1] == log_history['RO_M_mAP'].max():
                torch.save({
                    'epoch': epoch + 1,
                    'meta': model.meta,
                    'state_dict': model.state_dict(),
                    'min_loss': min_loss,
                    'optimizer': optimizer.state_dict(),
                }, os.path.join(args.directory, 'RO_M_mAP_best.pth.tar'))
            if 'RP_M_mAP' in log_history.columns.tolist() and \
                    log_history['RP_M_mAP'].values[-1] == log_history['RP_M_mAP'].max():
                torch.save({
                    'epoch': epoch + 1,
                    'meta': model.meta,
                    'state_dict': model.state_dict(),
                    'min_loss': min_loss,
                    'optimizer': optimizer.state_dict(),
                }, os.path.join(args.directory, 'RP_M_mAP_best.pth.tar'))
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        for q in range(nq):
            ...
            # hxq modified
            # output = model(input[q].cuda()).squeeze()
...
def test(datasets, net):
    ...
    for dataset in datasets:
        ...
        # PR_plot()
        # recall_at_N()
        _, map_record = compute_map_and_print(dataset, ranks, cfg['gnd'], kappas=[1, 5, 10, 100])
        map_record_all.update(map_record)
        ...
        if Lw is not None:
            ...
            ranks = np.argsort(-scores, axis=0)
            compute_map_and_print(dataset + ' + whiten', ranks, cfg['gnd'], kappas=[1, 5, 10, 100])

        print('>> {}: elapsed time: {}'.format(dataset, htime(time.time()-start)))

    return map_record_all
