...
for k in part:
    ...
    train_vecs = extract_vectors(net, images[num_list[k]:num_list[k+1]], image_size, transform, ms=ms, msp=msp)
    train_vecs = train_vecs.numpy()
    print('>>>> save train vecs to pkl...')
    train_vecs_file_path = os.path.join(get_data_root(), 'train_vecs{}_of_{}.pkl'.format(k+1, split_num))
    train_vecs_file = open(train_vecs_file_path, 'wb')
    pickle.dump(train_vecs, train_vecs_file)
    train_vecs_file.close()
    print('>>>> train_vecs{}_of_{}.pkl save done...'.format(k+1, split_num))
...
for i in range(split_num):
    ...
    with open(os.path.join(get_data_root(), 'train_vecs{}_of_{}.pkl'.format(i+1, split_num)), 'rb') as f:
...
if Lw is not None:
    ...
    split_num = 4
    for i in range(split_num):
        vecs_lw_temp = np.dot(Lw['P'], vecs[:, int(vecs.shape[1] / split_num * i):int(vecs.shape[1] / split_num * (i + 1))] - Lw['m'])
        if i == 0:
            vecs_lw = vecs_lw_temp
        else:
            vecs_lw = np.hstack((vecs_lw, vecs_lw_temp))
        del vecs_lw_temp
        gc.collect()
        print('\r>> index vecs PCA whitening {}/{} done...'.format(i+1, split_num), end='')
    print('')
    del vecs
    gc.collect()
...
for i in range(split_num):
    ...
    # todo: split train set to find knn, then stack the several knn to find knn again!!
...
# save to csv file
alpha = 3
rec_weight = ((np.arange(ranks.shape[0], 0, -1) / ranks.shape[0]) ** alpha)
rec_weight /= rec_weight.sum() # shape = (k, )
rec_landmarks = landmarks[ranks]  # shape = (k, query_num)

print(">> save to recognition_submission.csv file...")
submission_file = open(os.path.join(get_data_root(), 'recognition_submission.csv'), 'w')
...
for index, line in enumerate(csvreader):
    ...
    if flag == '1':
        for j in range(rec_landmarks.shape[0]):
            if landmark_dic.__contains__(rec_landmarks[j][cnt]):
                landmark_dic[rec_landmarks[j][cnt]] += rec_weight[j]
            else:
                landmark_dic[rec_landmarks[j][cnt]] = rec_weight[j]
        max_score_key = max(landmark_dic, key=landmark_dic.get)
        writer.writerow([img_name, '{} {}'.format(max_score_key, landmark_dic[max_score_key])])
        landmark_dic = {}
        cnt += 1
    else:
